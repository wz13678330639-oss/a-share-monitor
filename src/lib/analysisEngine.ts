...
function resolveConfidence(winRate: number) {
  if (winRate >= 86) return '胜率偏高'
...
function resolvePositionPct(
  ...
  const winBoost = Math.max(0, winRate - 70) * 0.55
  const riskPenalty = Math.max(0, riskScore - 45) * 0.48
  const cap = profile === 'low' ? 35 : profile === 'medium' ? 50 : 65

  return clamp(round(base + winBoost - riskPenalty), 5, cap)
}
    ...
    styleNote:
      profile === 'low'
        ...
        : profile === 'medium'
          ? '顺着趋势在回踩确认时参与，保留机动仓位。'
          : '只在强势延续时跟随，失速即撤。',
    positionPct: resolvePositionPct(profile, winRate, riskScore),
...
export function scoreStock(
  ...
  return {
    ...
    winRate,
    action,
    confidenceLabel: resolveConfidence(winRate),
    breakdown,
    tradePlan: buildTradePlan(stock, profile, winRate, riskScore),
...
export function buildDailyPicks(
  ...
  const scored = stocks
    .map((stock) => scoreStock(stock, profile))
...
export function buildAdaptiveReview(stocks: ScoredStock[], profile: RiskProfile) {
  if (!stocks.length) {
    return {
      ...
      learningScore: 0,
      bias: '等待自选股样本',
      summary: '当前自选股为空，系统会在加入股票后开始生成每日胜率复盘。',
      nextOptimization: '先建立自选池，再观察胜率、风险和信号结构。',
      focus: ['加入自选股', '等待行情同步', '建立复盘样本'],
    }
  }

  const avgWinRate = round(average(stocks.map((stock) => stock.winRate)))
  ...
  const highConfidenceCount = stocks.filter((stock) => stock.winRate >= 75).length
  ...
  const weak =
    ...
    '风险过滤'
  const confidence =
    avgWinRate >= 78 ? '强信号' : avgWinRate >= 68 ? '可跟踪' : '谨慎观察'
  const learningScore = clamp(
    round(avgWinRate * 0.62 + (100 - avgRisk) * 0.24 + highConfidenceCount * 3.5),
  )
  const bias =
    profile === 'low'
  ...
  return {
    ...
    learningScore,
    bias,
    summary: `今日自选池平均预测胜率 ${avgWinRate}%，高置信样本 ${highConfidenceCount}/${stocks.length}，平均风险 ${avgRisk}。`,
    nextOptimization:
      riskPressure > stocks.length / 2
        ? `风险压力偏高，系统下一轮会降低高波动标的权重，并提高${weak}的过滤要求。`
        : `风险压力可控，系统下一轮会优先保留${leader}强、且胜率连续稳定的标的。`,
    focus: [
...
export function buildMarketOverview(stocks: MarketStock[], profile: RiskProfile) {
  const scoredStocks = stocks.map((stock) => scoreStock(stock, profile))
  ...
  const avgWinRate = round(average(picks.map((item) => item.winRate)))
